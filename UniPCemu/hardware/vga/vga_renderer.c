...
void VGA_ActiveDisplay_noblanking_VGA(VGA_Type *VGA, SEQ_DATA *Sequencer, VGA_AttributeInfo *attributeinfo)
{
	...
	doublepixels = ((1<<((VGA->precalcs.ClockingModeRegister_DCR - ((VGA->precalcs.linearmode & 8) >> 3))))<<attributeinfo->attributesize); //Double the pixels(half horizontal clock) and multiply for each extra pixel clock taken?
