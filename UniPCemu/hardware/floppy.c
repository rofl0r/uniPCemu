...
FLOPPY_GEOMETRY floppygeometries[NUMFLOPPYGEOMETRIES] = { //Differently formatted disks, and their corresponding geometries
	//First, 5"
	{ 160,  8,  1, 40, FLOPPYTYPE_12MB, 0, TRANSFERRATE_250k|(TRANSFERRATE_300k<<2)|(TRANSFERRATE_300k<<4)|(TRANSFERRATE_300k<<6),0xFE,512 , 1, 64 ,DENSITY_SINGLE           ,GAPLENGTH_5_14,  0x00, 300}, //160K 5.25" supports 250kbits, 300kbits SD!
	{ 180,  9,  1, 40, FLOPPYTYPE_12MB, 0, TRANSFERRATE_250k|(TRANSFERRATE_300k<<2)|(TRANSFERRATE_300k<<4)|(TRANSFERRATE_300k<<6),0xFC,512 , 2, 64 ,DENSITY_SINGLE           ,GAPLENGTH_5_14,  0x00, 300}, //180K 5.25" supports 250kbits, 300kbits SD!
	{ 200, 10,  1, 40, FLOPPYTYPE_12MB, 0, TRANSFERRATE_250k|(TRANSFERRATE_300k<<2)|(TRANSFERRATE_300k<<4)|(TRANSFERRATE_300k<<6),0xFC,512 , 2, 64 ,DENSITY_SINGLE           ,GAPLENGTH_5_14,  0x00, 300}, //200K 5.25" supports 250kbits, 300kbits SD!
	{ 320,  8,  2, 40, FLOPPYTYPE_12MB, 0, TRANSFERRATE_250k|(TRANSFERRATE_300k<<2)|(TRANSFERRATE_300k<<4)|(TRANSFERRATE_300k<<6),0xFF,512 , 1, 112,DENSITY_SINGLE           ,GAPLENGTH_5_14,  0x00, 300}, //320K 5.25" supports 250kbits, 300kbits SD!
	{ 360,  9,  2, 40, FLOPPYTYPE_12MB, 0, TRANSFERRATE_250k|(TRANSFERRATE_300k<<2)|(TRANSFERRATE_300k<<4)|(TRANSFERRATE_300k<<6),0xFD,1024, 2, 112,DENSITY_DOUBLE           ,GAPLENGTH_5_14,  0x00, 300}, //360K 5.25" supports 250kbits, 300kbits DD!
	{ 400, 10,  2, 40, FLOPPYTYPE_12MB, 0, TRANSFERRATE_250k|(TRANSFERRATE_300k<<2)|(TRANSFERRATE_300k<<4)|(TRANSFERRATE_300k<<6),0xFD,1024, 2, 112,DENSITY_SINGLE           ,GAPLENGTH_5_14,  0x00, 300}, //400K 5.25" supports 250kbits, 300kbits SD!
	{1200, 15,  2, 80, FLOPPYTYPE_12MB, 0, TRANSFERRATE_250k|(TRANSFERRATE_300k<<2)|(TRANSFERRATE_500k<<4)|(TRANSFERRATE_500k<<6),0xF9,512 , 7, 224,DENSITY_SINGLE           ,GAPLENGTH_5_14,  0x00, 360}, //1200K 5.25" supports 300kbits, 500kbits SD!
	//Now 3.5"
	{ 720,  9,  2, 80, FLOPPYTYPE_720K, 1, TRANSFERRATE_250k|(TRANSFERRATE_300k<<2)|(TRANSFERRATE_300k<<4)|(TRANSFERRATE_300k<<6),0xF9,1024, 3, 112,DENSITY_DOUBLE           ,GAPLENGTH_3_5,   0xC0, 300 }, //720K 3.5" supports 250kbits, 300kbits DD! Disable gap length checking here because we need to work without it on a XT?
	{1440, 18,  2, 80, FLOPPYTYPE_14MB, 1, TRANSFERRATE_250k|(TRANSFERRATE_300k<<2)|(TRANSFERRATE_500k<<4)|(TRANSFERRATE_500k<<6),0xF0,512 , 9, 224,DENSITY_IGNORE|DENSITY_HD,GAPLENGTH_3_5,   0x80, 300 }, //1.44M 3.5" supports 250kbits, 500kbits HD! Disable gap length checking here because we need to work without it on a XT?
	{1680, 21,  2, 80, FLOPPYTYPE_14MB, 1, TRANSFERRATE_250k|(TRANSFERRATE_500k<<2)|(TRANSFERRATE_500k<<4)|(TRANSFERRATE_500k<<6),0xF0,512 , 9, 224,DENSITY_IGNORE|DENSITY_HD,GAPLENGTH_3_5,   0x80, 300 }, //1.68M 3.5" supports 250kbits, 500kbits HD! Supporting BIOS only!
	{1722, 21,  2, 82, FLOPPYTYPE_14MB, 1, TRANSFERRATE_250k|(TRANSFERRATE_500k<<2)|(TRANSFERRATE_500k<<4)|(TRANSFERRATE_500k<<6),0xF0,512 , 9, 224,DENSITY_IGNORE|DENSITY_HD,GAPLENGTH_3_5,   0x80, 300 }, //1.722M 3.5" supports 250kbits, 500kbits HD! Supporting BIOS only!
	{1840, 23,  2, 80, FLOPPYTYPE_14MB, 1, TRANSFERRATE_250k|(TRANSFERRATE_500k<<2)|(TRANSFERRATE_500k<<4)|(TRANSFERRATE_500k<<6),0xF0,512 , 9, 224,DENSITY_IGNORE|DENSITY_HD,GAPLENGTH_3_5,   0x80, 300 }, //1.84M 3.5" supports 250kbits, 500kbits HD! Supporting BIOS only!
	{2880, 36,  2, 80, FLOPPYTYPE_28MB, 1, TRANSFERRATE_1M|(TRANSFERRATE_1M<<2)|(TRANSFERRATE_1M<<4)|(TRANSFERRATE_1M<<6),        0xF0,1024, 9, 240,DENSITY_IGNORE|DENSITY_ED,GAPLENGTH_IGNORE,0x40, 300 } //2.88M 3.5" supports 1Mbits ED!
};
...
OPTINLINE void floppy_readsector() //Request a read sector command!
{
	...
	else //DSK or error?
	{
		...
		{
			if (readDSKSectorData(DSKImageFile,FLOPPY.currenthead[FLOPPY_DOR_DRIVENUMBERR], FLOPPY.currentcylinder[FLOPPY_DOR_DRIVENUMBERR], FLOPPY.currentsector[FLOPPY_DOR_DRIVENUMBERR], FLOPPY.commandbuffer[5], &FLOPPY.databuffersize)) //Read the data into memory?
			{
...
OPTINLINE void FLOPPY_formatsector() //Request a read sector command!
{
	...
	else //Writeable disk?
	{
		...
		else //Are we a normal image file?
		{
			...
			if (!writedata(FLOPPY_DOR_DRIVENUMBERR ? FLOPPY1 : FLOPPY0, &FLOPPY.databuffer, floppy_LBA(FLOPPY_DOR_DRIVENUMBERR, FLOPPY.currenthead[FLOPPY_DOR_DRIVENUMBERR], FLOPPY.currentcylinder[FLOPPY_DOR_DRIVENUMBERR], FLOPPY.currentsector[FLOPPY_DOR_DRIVENUMBERR]),512)) //Failed writing the formatted sector?
			{
				goto floppy_errorformat;
				return; //Error!
			}
...
OPTINLINE void floppy_executeCommand() //Execute a floppy command. Buffers are fully filled!
{
	...
	updateFloppyGeometries(FLOPPY_DOR_DRIVENUMBERR, FLOPPY.currenthead[FLOPPY_DOR_DRIVENUMBERR], FLOPPY.currentcylinder[FLOPPY_DOR_DRIVENUMBERR]); //Update the floppy geometries!
	switch (FLOPPY.commandbuffer[0]) //What command!
	{
		case WRITE_DATA: //Write sector
		...
		case READ_ID: //Read sector ID
			...
			if ((DSKImageFile = getDSKimage((FLOPPY_DOR_DRIVENUMBERR) ? FLOPPY1 : FLOPPY0))) //Are we a DSK image file?
			{
				if (readDSKSectorInfo(DSKImageFile, FLOPPY.currenthead[FLOPPY_DOR_DRIVENUMBERR], FLOPPY.currentcylinder[FLOPPY_DOR_DRIVENUMBERR], FLOPPY.currentsector[FLOPPY_DOR_DRIVENUMBERR], &sectorinformation)) //Read the sector information too!
				{
					FLOPPY.ST1 = sectorinformation.ST1; //Load ST1!
					FLOPPY.ST2 = sectorinformation.ST2; //Load ST2!
			...
			else //Normal disk? Generate valid data!
			{
				...
				{
					if ((int_32)floppy_LBA(FLOPPY_DOR_DRIVENUMBERR, FLOPPY.currenthead[FLOPPY_DOR_DRIVENUMBERR], FLOPPY.currentcylinder[FLOPPY_DOR_DRIVENUMBERR], FLOPPY.currentsector[FLOPPY_DOR_DRIVENUMBERR]) >= (int_32)(FLOPPY.geometries[FLOPPY_DOR_DRIVENUMBERR]->KB * 1024)) //Invalid address within our image!
					{
						FLOPPY_ST1_NOADDRESSMARKW(1);
