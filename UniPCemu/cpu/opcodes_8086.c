	...
	if (unlikely(CPU[activeCPU].internalinstructionstep==0)) //First step?
	{
		if (unlikely(CPU[activeCPU].internalmodrmstep==0))
		{
			if (reg==NULL) 
			{
				...
				if (modrm_check16(&params,MODRM_src0,1|0x30)) return 1; //Abort on fault!
	...
	if (unlikely(CPU[activeCPU].internalinstructionstep==0)) //First step?
	{
		if (unlikely(CPU[activeCPU].internalmodrmstep==0))
		{
			if (reg==NULL)
			{
				...
				if (modrm_check8(&params,MODRM_src0,1|0x30)) return 1; //Abort on fault!
				if (modrm_check8(&params,MODRM_src0,0|0x30)) return 1; //Abort on fault!
			}
		}
		if (reg==NULL) //Needs a read from memory?
		{
	...
	if (unlikely(CPU[activeCPU].internalinstructionstep==0)) //First step?
	{
		if (unlikely(CPU[activeCPU].internalmodrmstep==0))
		{
			if (dest == NULL)
			{
				...
				if (modrm_check16(&params, MODRM_src0, 1|0x30)) return 1; //Abort on fault!
				if (dest == NULL) if (modrm_check16(&params, MODRM_src0, 0|0x30)) return 1; //Abort on fault on write only!
			}
		}
		if (dest==NULL) //Needs a read from memory?
		{
	...
	if (unlikely(CPU[activeCPU].internalinstructionstep==0)) //First step?
	{
		if (unlikely(CPU[activeCPU].internalmodrmstep==0))
		{
			if (dest == NULL)
			{
				...
				if (modrm_check8(&params, MODRM_src0, 1|0x30)) return 1; //Abort on fault!
				if (dest == NULL) if (modrm_check8(&params, MODRM_src0, 0|0x30)) return 1; //Abort on fault on write only!
			}
		}
		if (dest==NULL) //Needs a read from memory?
		{
...
OPTINLINE byte CPU8086_internal_MOVSB()
{
	...
	if (unlikely(CPU[activeCPU].internalinstructionstep==0)) //First step?
	{
		...
		if (checkMMUaccess(CPU_segment_index(CPU_SEGMENT_DS), CPU_segment(CPU_SEGMENT_DS), (CPU_Address_size[activeCPU] ? REG_ESI : REG_SI), 1|0x30, getCPL(), !CPU_Address_size[activeCPU], 0)) //Error accessing memory?
		{
			return 1; //Abort on fault!
		}
		if (checkMMUaccess(CPU_SEGMENT_ES, REG_ES, (CPU_Address_size[activeCPU] ? REG_EDI : REG_DI), 0|0x30, getCPL(), !CPU_Address_size[activeCPU], 0)) //Error accessing memory?
		{
			return 1; //Abort on fault!
		}
		++CPU[activeCPU].internalinstructionstep; //Next step!
...
OPTINLINE byte CPU8086_internal_MOVSW()
{
	...
	if (unlikely(CPU[activeCPU].internalinstructionstep==0)) //First step?
	{
		...
		if (checkMMUaccess(CPU_segment_index(CPU_SEGMENT_DS), CPU_segment(CPU_SEGMENT_DS), (CPU_Address_size[activeCPU] ? REG_ESI : REG_SI), 1|0x30, getCPL(), !CPU_Address_size[activeCPU], 0 | 0x8)) //Error accessing memory?
		{
			return 1; //Abort on fault!
		}
		if (checkMMUaccess(CPU_segment_index(CPU_SEGMENT_DS), CPU_segment(CPU_SEGMENT_DS), (CPU_Address_size[activeCPU] ? REG_ESI : REG_SI) + 1, 1|0x30, getCPL(), !CPU_Address_size[activeCPU], 1 | 0x8)) //Error accessing memory?
		{
			return 1; //Abort on fault!
		}
		if (checkMMUaccess(CPU_SEGMENT_ES, REG_ES, (CPU_Address_size[activeCPU] ? REG_EDI : REG_DI), 0|0x30, getCPL(), !CPU_Address_size[activeCPU], 0 | 0x8)) //Error accessing memory?
		{
			return 1; //Abort on fault!
		}
		if (checkMMUaccess(CPU_SEGMENT_ES, REG_ES, (CPU_Address_size[activeCPU] ? REG_EDI : REG_DI) + 1, 0|0x30, getCPL(), !CPU_Address_size[activeCPU], 1 | 0x8)) //Error accessing memory?
		{
			return 1; //Abort on fault!
		}
		++CPU[activeCPU].internalinstructionstep; //Next step!
...
OPTINLINE byte CPU8086_internal_CMPSB()
{
	...
	if (unlikely(CPU[activeCPU].internalinstructionstep==0)) //First step?
	{
		...
		if (checkMMUaccess(CPU_SEGMENT_ES, REG_ES, (CPU_Address_size[activeCPU] ? REG_EDI : REG_DI), 1|0x30, getCPL(), !CPU_Address_size[activeCPU], 0)) //Error accessing memory?
		{
			return 1; //Abort on fault!
		}
		++CPU[activeCPU].internalinstructionstep; //Next step!
...
OPTINLINE byte CPU8086_internal_CMPSW()
{
	...
	if (unlikely(CPU[activeCPU].internalinstructionstep==0)) //First step?
	{
		...
		if (checkMMUaccess(CPU_SEGMENT_ES, REG_ES, (CPU_Address_size[activeCPU] ? REG_EDI : REG_DI), 1|0x30, getCPL(), !CPU_Address_size[activeCPU], 0 | 0x8)) //Error accessing memory?
		{
			return 1; //Abort on fault!
		}
		if (checkMMUaccess(CPU_SEGMENT_ES, REG_ES, (CPU_Address_size[activeCPU] ? REG_EDI : REG_DI) + 1, 1|0x30, getCPL(), !CPU_Address_size[activeCPU], 1 | 0x8)) //Error accessing memory?
		{
			return 1; //Abort on fault!
		}
		++CPU[activeCPU].internalinstructionstep; //Next step!
...
OPTINLINE byte CPU8086_internal_XCHG8(byte *data1, byte *data2, byte flags)
{
	if (unlikely(CPU[activeCPU].internalinstructionstep==0))
	{
		...
		if (data2==NULL)
		{
			...
			if (modrm_check8(&params,MODRM_src1,1|0x30)) return 1; //Abort on fault!
			if (modrm_check8(&params,MODRM_src1,0|0x30)) return 1; //Abort on fault!
		}
		++CPU[activeCPU].internalinstructionstep; //Next internal instruction step!
	}
	CPUPROT1
...
OPTINLINE byte CPU8086_internal_XCHG16(word *data1, word *data2, byte flags)
{
	if (unlikely(CPU[activeCPU].internalinstructionstep==0))
	{
		...
		if (data2==NULL)
		{
			...
			if (modrm_check16(&params,MODRM_src1,0|0x30)) return 1; //Abort on fault!
		}
		++CPU[activeCPU].internalinstructionstep; //Next internal instruction step!
	}
	CPUPROT1
...
byte CPU8086_internal_LXS(int segmentregister) //LDS, LES etc.
{
	...
	if (unlikely(CPU[activeCPU].internalinstructionstep==0))
	{
		...
		if (modrm_check16(&params,MODRM_src1,1|0x30)) return 1; //Abort on fault!
		modrm_addoffset = 0;
		if (modrm_check16(&params,MODRM_src0,0|0x30)) return 1; //Abort on fault for the used segment itself!
		++CPU[activeCPU].internalinstructionstep; //Next internal instruction step!
	}
	CPUPROT1
	if (CPU[activeCPU].internalinstructionstep==1) //First step?
...
void CPU8086_execute_ADD_modrmmodrm16() { modrm_generateInstructionTEXT("ADD", 16, 0, PARAM_MODRM_01); if (unlikely(CPU[activeCPU].modrmstep == 0)) { if (modrm_check16(&params, MODRM_src1, 1|0x40)) return; if (modrm_check16(&params, MODRM_src1, 1|0x30)) return; } if (CPU8086_instructionstepreadmodrmw(0, &instructionbufferw, MODRM_src1)) return; CPU8086_internal_ADD16(modrm_addr16(&params, MODRM_src0, 0), instructionbufferw, 2); }
...
void CPU8086_execute_OR_modrmmodrm16() { modrm_generateInstructionTEXT("OR", 16, 0, PARAM_MODRM_01); if (unlikely(CPU[activeCPU].modrmstep == 0)) { if (modrm_check16(&params, MODRM_src1, 1|0x40)) return; if (modrm_check16(&params, MODRM_src1, 1|0x30)) return; } if (CPU8086_instructionstepreadmodrmw(0, &instructionbufferw, MODRM_src1)) return; CPU8086_internal_OR16(modrm_addr16(&params, MODRM_src0, 0), instructionbufferw, 2); }
...
void CPU8086_execute_ADC_modrmmodrm16() { modrm_generateInstructionTEXT("ADC", 16, 0, PARAM_MODRM_01); if (unlikely(CPU[activeCPU].modrmstep == 0)) { if (modrm_check16(&params, MODRM_src1, 1|0x40)) return; if (modrm_check16(&params, MODRM_src1, 1|0x30)) return; } if (CPU8086_instructionstepreadmodrmw(0, &instructionbufferw, MODRM_src1)) return; CPU8086_internal_ADC16(modrm_addr16(&params, MODRM_src0, 0), instructionbufferw, 2); }
...
void CPU8086_execute_SBB_modrmmodrm16() { modrm_generateInstructionTEXT("SBB", 16, 0, PARAM_MODRM_01); if (unlikely(CPU[activeCPU].modrmstep == 0)) { if (modrm_check16(&params, MODRM_src1, 1|0x40)) return; if (modrm_check16(&params, MODRM_src1, 1|0x30)) return; } if (CPU8086_instructionstepreadmodrmw(0, &instructionbufferw, MODRM_src1)) return; CPU8086_internal_SBB16(modrm_addr16(&params, MODRM_src0, 0), instructionbufferw, 2); }
...
void CPU8086_execute_AND_modrmmodrm16() { modrm_generateInstructionTEXT("AND", 16, 0, PARAM_MODRM_01); if (unlikely(CPU[activeCPU].modrmstep == 0)) { if (modrm_check16(&params, MODRM_src1, 1|0x40)) return; if (modrm_check16(&params, MODRM_src1, 1|0x30)) return; } if (CPU8086_instructionstepreadmodrmw(0, &instructionbufferw, MODRM_src1)) return; CPU8086_internal_AND16(modrm_addr16(&params, MODRM_src0, 0), instructionbufferw, 2); }
...
void CPU8086_execute_SUB_modrmmodrm16() { modrm_generateInstructionTEXT("SUB", 16, 0, PARAM_MODRM_01); if (unlikely(CPU[activeCPU].modrmstep == 0)) { if (modrm_check16(&params, MODRM_src1, 1|0x40)) return; if (modrm_check16(&params, MODRM_src1, 1|0x30)) return; } if (CPU8086_instructionstepreadmodrmw(0, &instructionbufferw, MODRM_src1)) return; CPU8086_internal_SUB16(modrm_addr16(&params, MODRM_src0, 0), instructionbufferw, 2); }
...
void CPU8086_execute_XOR_modrmmodrm16() { modrm_generateInstructionTEXT("XOR", 16, 0, PARAM_MODRM_01); if (unlikely(CPU[activeCPU].modrmstep == 0)) { if (modrm_check16(&params, MODRM_src1, 1|0x40)) return; if (modrm_check16(&params, MODRM_src1, 1|0x30)) return; } if (CPU8086_instructionstepreadmodrmw(0, &instructionbufferw, MODRM_src1)) return; CPU8086_internal_XOR16(modrm_addr16(&params, MODRM_src0, 0), instructionbufferw, 2); }
...
void CPU8086_execute_CMP_modrmmodrm8() {modrm_generateInstructionTEXT("CMP",8,0,PARAM_MODRM_01); if (unlikely(CPU[activeCPU].modrmstep==0)) { if (modrm_check8(&params,MODRM_src0,1|0x40)) return; if (modrm_check8(&params,MODRM_src1,1|0x40)) return; if (modrm_check8(&params,MODRM_src0,1|0x30)) return; if (modrm_check8(&params,MODRM_src1,1|0x30)) return; } if (CPU8086_instructionstepreadmodrmb(0,&instructionbufferb,MODRM_src0)) return; if (CPU8086_instructionstepreadmodrmb(2,&instructionbufferb2,MODRM_src1)) return; CMP_b(instructionbufferb,instructionbufferb2,2); }
void CPU8086_execute_CMP_modrmmodrm16() {modrm_generateInstructionTEXT("CMP",16,0,PARAM_MODRM_01); if (unlikely(CPU[activeCPU].modrmstep==0)) { if (modrm_check16(&params,MODRM_src0,1|0x40)) return; if (modrm_check16(&params,MODRM_src1,1|0x40)) return; if (modrm_check16(&params,MODRM_src0,1|0x30)) return; if (modrm_check16(&params,MODRM_src1,1|0x30)) return; } if (CPU8086_instructionstepreadmodrmw(0,&instructionbufferw,MODRM_src0)) return; if (CPU8086_instructionstepreadmodrmw(2,&instructionbufferw2,MODRM_src1)) return; CMP_w(instructionbufferw,instructionbufferw2,2); }
...
void CPU8086_OP84() {modrm_generateInstructionTEXT("TEST",8,0,PARAM_MODRM_01); if (unlikely(CPU[activeCPU].modrmstep==0)) { if (modrm_check8(&params,MODRM_src0,1|0x40)) return; if (modrm_check8(&params,MODRM_src1,1|0x40)) return; if (modrm_check8(&params,MODRM_src0,1|0x30)) return; if (modrm_check8(&params,MODRM_src1,1|0x30)) return; } if (CPU8086_instructionstepreadmodrmb(0,&instructionbufferb,MODRM_src0)) return; if (CPU8086_instructionstepreadmodrmb(2,&instructionbufferb2,MODRM_src1)) return; CPU8086_internal_TEST8(instructionbufferb,instructionbufferb2,2); }
void CPU8086_OP85() {modrm_generateInstructionTEXT("TEST",16,0,PARAM_MODRM_01); if (unlikely(CPU[activeCPU].modrmstep==0))  { if (modrm_check16(&params,MODRM_src0,1|0x40)) return; if (modrm_check16(&params,MODRM_src1,1|0x40)) return; if (modrm_check16(&params,MODRM_src0,1|0x30)) return; if (modrm_check16(&params,MODRM_src1,1|0x30)) return; } if (CPU8086_instructionstepreadmodrmw(0,&instructionbufferw,MODRM_src0)) return; if (CPU8086_instructionstepreadmodrmw(2,&instructionbufferw2,MODRM_src1)) return; CPU8086_internal_TEST16(instructionbufferw,instructionbufferw2,2); }
...
void CPU8086_execute_MOV_modrmmodrm16() { modrm_generateInstructionTEXT("MOV", 16, 0, PARAM_MODRM_01); if (unlikely(CPU[activeCPU].modrmstep == 0)) { if (modrm_check16(&params, MODRM_src1, 1|0x40)) return; if (modrm_check16(&params, MODRM_src1, 1|0x30)) return; } if (CPU8086_instructionstepreadmodrmw(0, &instructionbufferw, MODRM_src1)) return; CPU8086_internal_MOV16(modrm_addr16(&params, MODRM_src0, 0), instructionbufferw, 2); }
void CPU8086_execute_MOVSegRegMemory() { modrm_generateInstructionTEXT("MOV", 16, 0, PARAM_MODRM_01); if (unlikely(CPU[activeCPU].modrmstep == 0)) { if (modrm_check16(&params, MODRM_src1, 1|0x40)) return; if (modrm_check16(&params, MODRM_src1, 1|0x30)) return; } if (CPU8086_instructionstepreadmodrmw(0, &instructionbufferw, MODRM_src1)) return; if (CPU8086_internal_MOV16(modrm_addr16(&params, MODRM_src0, 0), instructionbufferw, 8)) return; if ((params.info[MODRM_src0].reg16 == &CPU[activeCPU].registers->SS) && (params.info[MODRM_src0].isreg == 1)) { CPU[activeCPU].allowInterrupts = 0; /* Inhabit all interrupts up to the next instruction */ } }
...
void CPU8086_OPA1() {INLINEREGISTER uint_32 theimm = immaddr32; debugger_setcommand("MOV AX,word %s:[%04X]",CPU_textsegment(CPU_SEGMENT_DS),theimm);/*MOV AX,[imm16]*/ if (unlikely(CPU[activeCPU].modrmstep==0)) {  if (checkMMUaccess(CPU_segment_index(CPU_SEGMENT_DS),CPU_segment(CPU_SEGMENT_DS),(theimm&CPU[activeCPU].address_size),1|0x40,getCPL(),1,0|0x8)) return; if (checkMMUaccess(CPU_segment_index(CPU_SEGMENT_DS),CPU_segment(CPU_SEGMENT_DS),(theimm&CPU[activeCPU].address_size)+1,1|0x40,getCPL(),1,1|0x8)) return; if (checkMMUaccess(CPU_segment_index(CPU_SEGMENT_DS), CPU_segment(CPU_SEGMENT_DS), (theimm&CPU[activeCPU].address_size), 1|0x30, getCPL(), 1, 0 | 0x8)) return; if (checkMMUaccess(CPU_segment_index(CPU_SEGMENT_DS), CPU_segment(CPU_SEGMENT_DS), (theimm&CPU[activeCPU].address_size) + 1, 1|0x30, getCPL(), 1, 1 | 0x8)) return; } if (CPU8086_instructionstepreaddirectw(0,CPU_segment_index(CPU_SEGMENT_DS),CPU_segment(CPU_SEGMENT_DS),theimm,&instructionbufferw,1)) return; CPU8086_internal_MOV16(&REG_AX,instructionbufferw,1);/*MOV AX,[imm16]*/ }
...
void CPU8086_OPC7() { word val = immw; modrm_debugger16(&params, MODRM_src0, MODRM_src1); debugger_setcommand("MOV %s,%04x", modrm_param1, val); if (unlikely(CPU[activeCPU].modrmstep == 0)) { if (modrm_check16(&params, MODRM_src0, 0|0x40)) return; if (modrm_check16(&params, MODRM_src0, 0|0x30)) return; } if (CPU8086_instructionstepwritemodrmw(0, val, MODRM_src0, 0)) return; if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { if (MODRM_EA(params)) { CPU[activeCPU].cycles_OP += 3; /* Imm->Mem */ } else CPU[activeCPU].cycles_OP += 2; /* Imm->Reg */ } }
...
void CPU8086_OP81() //GRP1 Ev,Iv
{
	...
	case 7: //CMP
		...
		if (unlikely(CPU[activeCPU].modrmstep == 0)) { if (modrm_check16(&params, MODRM_src0, 1|0x40)) return; if (modrm_check16(&params, MODRM_src0, 1|0x30)) return; } //Abort when needed!
...
void CPU8086_OP8F() //Undocumented GRP opcode 8F r/m16
{
	...
	case 0: //POP
		...
		if (unlikely(CPU[activeCPU].instructionstep == 0)) { if (modrm_check16(&params, MODRM_src0, 0|0x40)) return; if (modrm_check16(&params, MODRM_src0, 0|0x30)) return; } //Abort when needed!
		static word value;
		//Execution step!
...
void op_grp5() {
	...
	case 5: //JMP Mp
		...
		if (unlikely(CPU[activeCPU].modrmstep==2)) //Starting and to check?
		{
			...
			if (checkMMUaccess(get_segment_index(info.segmentregister), info.mem_segment, (info.mem_offset&CPU[activeCPU].address_size), 1|0x30, getCPL(), !CPU_Address_size[activeCPU], 0 | 0x8)) return; //Abort on fault!
			if (checkMMUaccess(get_segment_index(info.segmentregister), info.mem_segment, (info.mem_offset&CPU[activeCPU].address_size) + 1, 1|0x30, getCPL(), !CPU_Address_size[activeCPU], 1 | 0x8)) return; //Abort on fault!
			if (checkMMUaccess(get_segment_index(info.segmentregister), info.mem_segment, ((info.mem_offset + 2)&info.memorymask), 1|0x30, getCPL(), !CPU_Address_size[activeCPU], 0 | 0x8)) return; //Abort on fault!
			if (checkMMUaccess(get_segment_index(info.segmentregister), info.mem_segment, ((info.mem_offset + 2)&info.memorymask) + 1, 1|0x30, getCPL(), !CPU_Address_size[activeCPU], 1 | 0x8)) return; //Abort on fault!
		}
