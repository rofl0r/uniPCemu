...
OPTINLINE void CPU186_internal_MOV16(word *dest, word val) //Copy of 8086 version!
{
	CPUPROT1
		...
		else //Memory?
		{
			if (custommem)
			{
				if (checkMMUaccess(CPU_segment_index(CPU_SEGMENT_DS), CPU_segment(CPU_SEGMENT_DS), (customoffset&CPU[activeCPU].address_size),0,getCPL(),!CPU_Address_size[activeCPU],0|0x8)) return; //Abort on fault!
				if (checkMMUaccess(CPU_segment_index(CPU_SEGMENT_DS), CPU_segment(CPU_SEGMENT_DS), (customoffset&CPU[activeCPU].address_size)+1,0,getCPL(),!CPU_Address_size[activeCPU],1|0x8)) return; //Abort on fault!
...
void CPU186_OP6C()
{
	...
	CPUPROT1
	if (CPU8086_instructionstepwritedirectb(0,CPU_segment_index(CPU_SEGMENT_ES),CPU_segment(CPU_SEGMENT_ES),(CPU_Address_size[activeCPU]?REG_EDI:REG_DI),data,!CPU_Address_size[activeCPU])) return; //INSB
	CPUPROT1
	if (FLAG_DF)
	{
...
void CPU186_OP6D()
{
	...
	CPUPROT1
	if (CPU8086_instructionstepwritedirectw(0,CPU_segment_index(CPU_SEGMENT_ES),CPU_segment(CPU_SEGMENT_ES),(CPU_Address_size[activeCPU]?REG_EDI:REG_DI),data,!CPU_Address_size[activeCPU])) return; //INSW
	CPUPROT1
	if (FLAG_DF)
	{
		if (CPU_Address_size[activeCPU])
