...
void CPU486_OP0FB1_16() {word temp; if (modrm_check16(&params,MODRM_src0,1|0x40)) return; if (modrm_check16(&params,MODRM_src0,1|0x30)) return; temp = modrm_read16(&params,MODRM_src0); if (REG_AX==temp) { if (modrm_check16(&params,MODRM_src0,0|0x40)) return; if (modrm_check16(&params,MODRM_src0,0|0x30)) return; FLAGW_ZF(1); modrm_write16(&params,MODRM_src0,modrm_read16(&params,MODRM_src1),0); /* r/m16=r16 */ } else { FLAGW_ZF(0); REG_AX = temp; /* AX=r/m16 */ }} //CMPXCHG r/m16,AX,r16
void CPU486_OP0FB1_32() {uint_32 temp; if (modrm_check32(&params,MODRM_src0,1|0x40)) return; if (modrm_check32(&params,MODRM_src0,1|0x30)) return; temp = modrm_read32(&params,MODRM_src0); if (REG_EAX==temp) { if (modrm_check32(&params,MODRM_src0,0|0x40)) return; if (modrm_check32(&params,MODRM_src0,0|0x30)) return; FLAGW_ZF(1); modrm_write32(&params,MODRM_src0,modrm_read32(&params,MODRM_src1)); /* r/m32=r32 */ } else { FLAGW_ZF(0); REG_EAX = temp; /* EAX=r/m32 */ }} //CMPXCHG r/m32,EAX,r32
...
void CPU486_OP0FC1_16() {modrm_generateInstructionTEXT("XADD",16,0,PARAM_MODRM21); if (modrm_check16(&params,MODRM_src0,0|0x40)) return; if (modrm_check16(&params,MODRM_src0,0|0x30)) return; oper1 = modrm_read16(&params,MODRM_src1); oper2 = modrm_read16(&params,MODRM_src0); op_add16_486(); modrm_write16(&params,MODRM_src1,oper2,0); modrm_write16(&params,MODRM_src0,res16,0);} //XADD r/m16,r16
void CPU486_OP0FC1_32() {modrm_generateInstructionTEXT("XADD",32,0,PARAM_MODRM21); if (modrm_check32(&params,MODRM_src0,0|0x40)) return; if (modrm_check32(&params,MODRM_src0,0|0x30)) return; oper1d = modrm_read32(&params,MODRM_src1); oper2d = modrm_read32(&params,MODRM_src0); op_add32_486(); modrm_write32(&params,MODRM_src1,oper2d); modrm_write32(&params,MODRM_src0,res32);} //XADD r/m32,r32
