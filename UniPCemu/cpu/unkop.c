...
void unkOP_8086() //Unknown opcode on 8086?
{
	...
	dolog("unkOP","Unknown 8086 opcode detected: %02X@%04X:%04X, Previous opcode: %02X@%04X:%04X",CPU[activeCPU].lastopcode,CPU_exec_CS,CPU_exec_EIP,CPU[activeCPU].previousopcode,CPU_exec_lastCS,CPU_exec_lastEIP); //Log our info!
...
void unkOP_186() //Unknown opcode on 186+?
{
	...
	#ifdef UNKOP_SHUTDOWN
	dolog("unkOP","Unknown opcode detected: %02X@%04X:%08X, Previous opcode: %02X@%04X:%08X",CPU[activeCPU].lastopcode,CPU_exec_CS,CPU_exec_EIP,CPU[activeCPU].previousopcode,CPU_exec_lastCS,CPU_exec_lastEIP); //Log our info!
	EMU_Shutdown(1); //Request to shut down!
	#endif
}
...
void unkOP0F_286() //0F unknown opcode handler on 286+?
{
	...
	CPU[activeCPU].faultraised = 1; //We've raised a fault!
	#ifdef UNKOP_SHUTDOWN
	dolog("unkOP","Unknown 0F opcode detected: %02X@%04X:%08X, Previous opcode: %02X@%04X:%08X",CPU[activeCPU].lastopcode,CPU_exec_CS,CPU_exec_EIP,CPU[activeCPU].previousopcode,CPU_exec_lastCS,CPU_exec_lastEIP); //Log our info!
	EMU_Shutdown(1); //Request to shut down!
	#endif
}
