...
byte checkloadTSS16()
{
	...
	{
		debugger_forceimmediatelogging = 1; //Log!
		if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, n, 1|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
	}
...
byte checkloadTSS32()
{
	...
	debugger_forceimmediatelogging = 1; //Log!
	if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, 0, 1|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
	if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, 1, 1|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
	//SP0/ESP0 initializing!
	n = 4; //Start of our block!
	...
	{
		debugger_forceimmediatelogging = 1; //Log!
		if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, n + 0, 1|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
		if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, n + 1, 1|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
		if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, n + 2, 1|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
		if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, n + 3, 1|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
	...
	debugger_forceimmediatelogging = 1; //Log!
	if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, (25 * 4) + 0, 1|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
	if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, (25 * 4) + 1, 1|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
	debugger_forceimmediatelogging = 1; //Log!
	if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, (25 * 4) + 2, 1|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
	if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, (25 * 4) + 3, 1|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
	debugger_forceimmediatelogging = 0; //Don't log!
...
byte checksaveTSS16()
{
	...
	{
		debugger_forceimmediatelogging = 1; //Log!
		if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, n + 0, 0|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
		if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, n + 1, 0|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
	}
	debugger_forceimmediatelogging = 0; //Don't log!
	return 0; //OK!
}
...
byte checksaveTSS32()
{
	...
	for (n = (8 * 4); n < ((8 + 10) * 4); n += 4) //Write our TSS 32-bit data! Ignore the Stack data for different privilege levels and CR3(PDBR)!
	{
		...
		if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, n + 0, 0|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
		if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, n + 1, 0|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
		if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, n + 2, 0|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
		if (checkMMUaccess(CPU_SEGMENT_TR, CPU[activeCPU].registers->TR, n + 3, 0|0xA0, 0, 0, 0)) { debugger_forceimmediatelogging = 0; return 1; } //Error out!
	}
