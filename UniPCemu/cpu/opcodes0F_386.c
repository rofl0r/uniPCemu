...
void CPU80386_OP0F86_16() { INLINEREGISTER sword rel16;/*JNA rel16 : (CF=1|ZF=1)*/ rel16 = imm16(); modrm_generateInstructionTEXT("JB", 0, REG_EIP + rel16, CPU_EIPSize()); /* JUMP to destination? */ if (FLAG_CF || FLAG_ZF) { CPU_JMPrel(rel16); /* JUMP to destination? */ CPU_flushPIQ(-1); /*We're jumping to another address*/  didJump = 1; if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { CPU[activeCPU].cycles_OP += 16; CPU[activeCPU].cycles_stallBIU += CPU[activeCPU].cycles_OP; /*Stall the BIU completely now!*/ } /* Branch taken */ } else { if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { CPU[activeCPU].cycles_OP += 4; /* Branch not taken */ } } }
void CPU80386_OP0F87_16() { INLINEREGISTER sword rel16;/*JA rel16: (CF=0&ZF=0)*/ rel16 = imm16(); modrm_generateInstructionTEXT("JNBE", 0, REG_EIP + rel16, CPU_EIPSize()); /* JUMP to destination? */ if (!(FLAG_CF | FLAG_ZF)) { CPU_JMPrel(rel16); /* JUMP to destination? */ CPU_flushPIQ(-1); /*We're jumping to another address*/  didJump = 1; if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { CPU[activeCPU].cycles_OP += 16; CPU[activeCPU].cycles_stallBIU += CPU[activeCPU].cycles_OP; /*Stall the BIU completely now!*/ } /* Branch taken */ } else { if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { CPU[activeCPU].cycles_OP += 4; /* Branch not taken */ } } }
...
void CPU80386_OP0F8D_16() { INLINEREGISTER sword rel16;/*JGE rel16 : (SF=OF)*/ rel16 = imm16(); modrm_generateInstructionTEXT("JGE", 0, REG_EIP + rel16, CPU_EIPSize()); /* JUMP to destination? */ if (FLAG_SF == FLAG_OF) { CPU_JMPrel(rel16); /* JUMP to destination? */ CPU_flushPIQ(-1); /*We're jumping to another address*/  didJump = 1; if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { CPU[activeCPU].cycles_OP += 16; CPU[activeCPU].cycles_stallBIU += CPU[activeCPU].cycles_OP; /*Stall the BIU completely now!*/ } /* Branch taken */ } else { if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { CPU[activeCPU].cycles_OP += 4; /* Branch not taken */ } } }
...
void CPU80386_OP0F86_32() { INLINEREGISTER sword rel32;/*JNA rel32 : (CF=1|ZF=1)*/ rel32 = imm32s(); modrm_generateInstructionTEXT("JB", 0, REG_EIP + rel32, CPU_EIPSize()); /* JUMP to destination? */ if (FLAG_CF || FLAG_ZF) { CPU_JMPrel(rel32); /* JUMP to destination? */ CPU_flushPIQ(-1); /*We're jumping to another address*/  didJump = 1; if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { CPU[activeCPU].cycles_OP += 16; CPU[activeCPU].cycles_stallBIU += CPU[activeCPU].cycles_OP; /*Stall the BIU completely now!*/ } /* Branch taken */ } else { if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { CPU[activeCPU].cycles_OP += 4; /* Branch not taken */ } } }
void CPU80386_OP0F87_32() { INLINEREGISTER sword rel32;/*JA rel32: (CF=0&ZF=0)*/ rel32 = imm32s(); modrm_generateInstructionTEXT("JNBE", 0, REG_EIP + rel32, CPU_EIPSize()); /* JUMP to destination? */ if (!(FLAG_CF | FLAG_ZF)) { CPU_JMPrel(rel32); /* JUMP to destination? */ CPU_flushPIQ(-1); /*We're jumping to another address*/  didJump = 1; if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { CPU[activeCPU].cycles_OP += 16; CPU[activeCPU].cycles_stallBIU += CPU[activeCPU].cycles_OP; /*Stall the BIU completely now!*/ } /* Branch taken */ } else { if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { CPU[activeCPU].cycles_OP += 4; /* Branch not taken */ } } }
...
void CPU80386_OP0F8D_32() { INLINEREGISTER sword rel32;/*JGE rel32 : (SF=OF)*/ rel32 = imm32s(); modrm_generateInstructionTEXT("JGE", 0, REG_EIP + rel32, CPU_EIPSize()); /* JUMP to destination? */ if (FLAG_SF == FLAG_OF) { CPU_JMPrel(rel32); /* JUMP to destination? */ CPU_flushPIQ(-1); /*We're jumping to another address*/  didJump = 1; if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { CPU[activeCPU].cycles_OP += 16; CPU[activeCPU].cycles_stallBIU += CPU[activeCPU].cycles_OP; /*Stall the BIU completely now!*/ } /* Branch taken */ } else { if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { CPU[activeCPU].cycles_OP += 4; /* Branch not taken */ } } }
