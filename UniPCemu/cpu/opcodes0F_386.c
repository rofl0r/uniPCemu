...
void CPU80386_OP0F83_16() { INLINEREGISTER sword rel16;/*JNC rel16 : (CF=0)*/ rel16 = imm16(); modrm_generateInstructionTEXT("JNB", 0, REG_EIP + rel16, CPU_EIPSize()); /* JUMP to destination? */ if (!FLAG_CF) { CPU_JMPrel(rel16); /* JUMP to destination? */ CPU_flushPIQ(-1); /*We're jumping to another address*/ didJump = 1; if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { CPU[activeCPU].cycles_OP += 16; CPU[activeCPU].cycles_stallBIU += CPU[activeCPU].cycles_OP; /*Stall the BIU completely now!*/ } /* Branch taken */ } else { if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { CPU[activeCPU].cycles_OP += 4; /* Branch not taken */ } } }
...
void CPU80386_OP0F83_32() { INLINEREGISTER sword rel32;/*JNC rel32 : (CF=0)*/ rel32 = imm32s(); modrm_generateInstructionTEXT("JNB", 0, REG_EIP + rel32, CPU_EIPSize()); /* JUMP to destination? */ if (!FLAG_CF) { CPU_JMPrel(rel32); /* JUMP to destination? */ CPU_flushPIQ(-1); /*We're jumping to another address*/ didJump = 1; if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { CPU[activeCPU].cycles_OP += 16; CPU[activeCPU].cycles_stallBIU += CPU[activeCPU].cycles_OP; /*Stall the BIU completely now!*/ } /* Branch taken */ } else { if (CPU_apply286cycles() == 0) /* No 80286+ cycles instead? */ { CPU[activeCPU].cycles_OP += 4; /* Branch not taken */ } } }
