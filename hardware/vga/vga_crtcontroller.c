...
{
	if ((VGA->registers->specialCGAflags&0x81)==1) return get_display_CGA_y(VGA,scanline); //Give CGA timing!
	word signal;
...
{
	if ((VGA->registers->specialCGAflags&0x81)==1) return get_display_CGA_x(VGA,x); //Give CGA timing!
	word signal;
	signal = VGA_OVERSCAN; //Init to overscan!
