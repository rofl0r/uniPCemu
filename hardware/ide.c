...
byte ATA_readsector(byte channel, byte command) //Read the current sector set up!
{
...
void ATA_updateStatus(byte channel)
{
	...
	case 0: //Ready for command?
		ATA[channel].Drive[ATA_activeDrive(channel)].STATUSREGISTER.data = 0; //Clear all info!
...
void ATA_DiskChanged(int disk)
{
	...
	case CDROM1: //CDROM1 changed?
		if (has_drive(disk)) //Do we even have this drive?
		{
			...
			ATA[disk_channel].Drive[disk_ATA].driveparams[21] = 1; //Buffer size in sectors!
