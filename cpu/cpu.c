...
byte getcpumode() //Retrieves the current mode!
{
	...
	else if (CPU.registers->SFLAGS.V8) //VM86 mode when V8 bit set?
	{
		return CPU_MODE_8086; //Default: virtual 8086 mode!
	}
	//Real mode by default! This can't be reached in 8086/80186 real mode (PE&V8 flags unaccessable)!
			...
			if (CPU_getprefix(0xF2) || CPU_getprefix(0xF3)) //We're executing a REP instruction?
			{
				if (!CPU.faultraised) //No fault has been raised?
				{
					if (!blockREP) //Not starting empty?
					{
						...
						if (CPU_getprefix(0xF2)) //REPNZ?
						{
							if (!CPU.registers->SFLAGS.ZF) //To reset the opcode (ZF quits the loop)?
							{
						...
						else if (CPU_getprefix(0xF3))
						{
							if (REPZ) //REPZ?
							{
								if (CPU.registers->SFLAGS.ZF) //To reset the opcode (no ZF quits the loop)?
								{
									if (CPU.registers->CX) //Left to run?
									{
										CPU_resetOP(); //Retry!
