...
//Now, jump to it!
		REG_CS = MMU_rw(CB_ISCallback()?CPU_segment_index(CPU_SEGMENT_DS):-1,0x0000,(intnr<<2)|2,0); //JUMP to position REG_CS:REG_EIP/REG_CS:REG_IP in table.
		REG_IP = MMU_rw(CB_ISCallback()?CPU_segment_index(CPU_SEGMENT_DS):-1,0x0000,(intnr<<2),0); //JUMP to position REG_CS:REG_EIP/REG_CS:REG_IP in table.
	}
